...
from app.forms import RecipeForm, LoginForm, RegistrationForm, IngredientForm
...
def index():
	...
	next_url = url_for('index', page=recipeList.next_num) if recipeList.has_next else None
	prev_url = url_for('index', page=recipeList.prev_num) if recipeList.has_prev else None
...
def manage_ingredients():
	...
	next_url = url_for('manage_ingredients', page=ingredients.next_num) if ingredients.has_next else None
	prev_url = url_for('manage_ingredients', page=ingredients.prev_num) if ingredients.has_prev else None
	return render_template('manage_ingredients.html', Ingredients = ingredients.items, next_url = next_url, prev_url = prev_url)

@app.route('/admin/create/ingredient', methods=['GET','POST'])
@login_required
...
def manage_recipes():
	...
	next_url = url_for('manage_recipes', page=recipes.next_num) if recipes.has_next else None
	prev_url = url_for('manage_recipes', page=recipes.prev_num) if recipes.has_prev else None
...
@app.route('/admin/submit_recipe', methods=['GET','POST'])
def submit_recipe():
	if request.method == 'POST':
		...
		if recipeCheck is not None:
			...
			return redirect(url_for('admin_create_recipe'))
...
def manage_users():
	...
	next_url = url_for('manage_users', page=users.next_num) if users.has_next else None
	prev_url = url_for('manage_users', page=users.prev_num) if users.has_prev else None
